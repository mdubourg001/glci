...
async function main() {
  ...
  walker.start(); // handling inclusion of other yaml files
  ...
  if ("include" in ci) {
    ...
    for (const entry of ci.include) {
      if ("local" in entry) {
        included += fs__default['default'].readFileSync(process.cwd() + entry.local) + "\n";
      } else {
  ...
  for (const stage of ci.stages) {
    ...
    for (const name of jobs) {
      var _job$image, _ref, _job$cache$policy, _job$cache, _DEFAULT$cache, _job$cache$paths, _job$cache2, _DEFAULT$cache$paths, _DEFAULT$cache2, _job$stage;
      ...
      const workdir = `/${sha}`;
      const image = (_job$image = job.image) !== null && _job$image !== void 0 ? _job$image : DEFAULT.image;
      const cache = {
      ...
      const config = {
        ...
        HostConfig: {
          AutoRemove: true,
          Binds: [// binding project directory files as read-only
          ...projectFiles.map(p => `${path__default['default'].resolve(process.cwd(), p)}:${workdir}/${p}:ro`), // binding cache directories / files
          ...
          ...artifactsFiles.map(p => {
            var _job$artifacts, _job$artifacts$paths;

            return `${LOCAL_CI_DIR}/${p}:${p.startsWith("/") ? p : workdir + "/" + p}${(_job$artifacts = job.artifacts) !== null && _job$artifacts !== void 0 && (_job$artifacts$paths = _job$artifacts.paths) !== null && _job$artifacts$paths !== void 0 && _job$artifacts$paths.includes(p) ? "" : ":ro"}`;
          })]
        }
      };
      let container = null;
