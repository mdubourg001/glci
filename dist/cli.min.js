...
const LOCAL_CI_DIR = ARGV.dir ? path__default['default'].resolve(process.cwd(), ARGV.dir) : path__default['default'].resolve(process.cwd(), ".glci");
const LOCAL_CI_CACHE_DIR = path__default['default'].join(LOCAL_CI_DIR, ".glci_cache");
const LOCAL_CI_ARTIFACTS_DIR = path__default['default'].join(LOCAL_CI_DIR, ".glci_artifacts"); // keys unusable as job name because reserved
...
async function main() {
  ...
  const PROJECT_FILES_TEMP_DIR = path__default['default'].join(LOCAL_CI_DIR, sha);
  ...
  for (const stage of ci.stages) {
    ...
    for (const name of jobs) {
      var _job$image, _job$image2, _DEFAULT$image, _job$stage;
      ...
      if ("cache" in job) {
        var _job$cache$policy, _job$cache$paths;

        cache = {
          policy: (_job$cache$policy = job.cache.policy) !== null && _job$cache$policy !== void 0 ? _job$cache$policy : "pull-push",
          paths: (_job$cache$paths = job.cache.paths) !== null && _job$cache$paths !== void 0 ? _job$cache$paths : Array.isArray(job.cache) ? job.cache : []
        };
      } else {
        var _DEFAULT$cache$policy, _DEFAULT$cache, _DEFAULT$cache$paths, _DEFAULT$cache2;

        cache = {
          policy: (_DEFAULT$cache$policy = (_DEFAULT$cache = DEFAULT.cache) === null || _DEFAULT$cache === void 0 ? void 0 : _DEFAULT$cache.policy) !== null && _DEFAULT$cache$policy !== void 0 ? _DEFAULT$cache$policy : "pull-push",
          paths: (_DEFAULT$cache$paths = (_DEFAULT$cache2 = DEFAULT.cache) === null || _DEFAULT$cache2 === void 0 ? void 0 : _DEFAULT$cache2.paths) !== null && _DEFAULT$cache$paths !== void 0 ? _DEFAULT$cache$paths : Array.isArray(DEFAULT.cache) ? DEFAULT.cache : []
        };
      ...
      const onerror = async (err, container) => {
        ...
        fs__default['default'].removeSync(PROJECT_FILES_TEMP_DIR);
        process.exit(1);
      };
      ...
      for (const file of projectFiles) {
        mkdirpRecSync(path__default['default'].join(PROJECT_FILES_TEMP_DIR, path__default['default'].dirname(file)));
        fs__default['default'].copySync(`${path__default['default'].resolve(process.cwd(), file)}`, path__default['default'].join(PROJECT_FILES_TEMP_DIR, file), {
          recursive: true
        });
      }
      ...
      const config = {
        ...
        HostConfig: {
          AutoRemove: true,
          Binds: [// binding the copy of project directory
          `${PROJECT_FILES_TEMP_DIR}:${workdir}`, // binding cache directories / files
          // TODO: "Snapshot" and restore the _cache dir if policy === pull
          ...cache.paths.filter(p => fs__default['default'].existsSync(path__default['default'].join(LOCAL_CI_CACHE_DIR, p))).map(p => `${path__default['default'].join(LOCAL_CI_CACHE_DIR, p)}:${path__default['default'].join(workdir, p)}`) // binding dependencies
          // ...artifactsFiles.map(
          //   (p) =>
          //     `${LOCAL_CI_DIR}/${p}:${
      ...
      try {
        ...
        if (cache.policy !== "pull" && cache.paths.length > 0) {
          for (const file of cache.paths) {
            const fileAbs = path__default['default'].join(PROJECT_FILES_TEMP_DIR, file);
            const targetAbs = path__default['default'].join(LOCAL_CI_CACHE_DIR, file);

            if (fs__default['default'].existsSync(fileAbs)) {
              mkdirpRecSync(path__default['default'].dirname(targetAbs));
              fs__default['default'].copySync(fileAbs, targetAbs, {
                recursive: true
              });
            }
          }
        } // updating artifacts after job ended


        if ("artifacts" in job && job.artifacts.paths) {
          ARTIFACTS[name] = {};

          for (const file of job.artifacts.paths) {
            const fileAbs = path__default['default'].join(PROJECT_FILES_TEMP_DIR, file);
            const targetAbs = path__default['default'].join(LOCAL_CI_ARTIFACTS_DIR, `${name}_${file}`);
        ...
        fs__default['default'].removeSync(PROJECT_FILES_TEMP_DIR); // stopping container when finishing
